...
class user_login(tk.Tk):
	...
	def logwin(self):
		...
		tk.Button(page,text='退出',width=10,command=self.quit).grid(row=3,column=1,sticky='W',padx=10,pady=30)
...
class main_win(tk.Tk,object):
	...
	def mw(self):
		...
		tk.Label(ucondition,text='请选择交易类型:').grid(row=10,column=0,sticky='W',columnspan=4)#,pady=10
		...
		#退出按钮self.logout=
		tk.Button(self.mtable,text='退出',width=10,command=self.quit).grid(row=25,column=2,stick='W',padx=5,pady=10)
		#直接查询可用金额按钮
					...
					if self.optype in (1,2):
						if self.trancode in ('I','C','i','c'):
							...
							if col=='Error':
								...
								if data :
									self.scr.delete('1.0','end')
									self.scr.insert(tk.INSERT,'下列数据已经被删除：\n'+out)
